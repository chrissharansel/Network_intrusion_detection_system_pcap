...
"""
Advanced Network IDS v2.1 — Fixed Detection Logic
Key fixes:
  - Cooldown now works correctly (was always 0 due to missing initialization)
  - Brute force / DDoS / flood counters reset after alerting
  - Port scan window logic fixed
  - SYN flood resets correctly
  - Per-alert-type cooldown keys are consistent
"""
...
try:
    from scapy.all import (
        ...
        Raw, wrpcap, rdpcap, get_if_list, get_if_addr
    )
    SCAPY_AVAILABLE = True
    print("✅ Scapy loaded successfully")
except ImportError as e:
...
KNOWN_MALICIOUS_PORTS = {4444, 1337, 31337, 12345, 54321, 6666, 6667, 8888, 9999, 65000, 65535}
...
DEFAULT_CONFIG = {
    ...
    "thresholds": {
        ...
        "ddos_pps":              1000,   # packets/sec from one IP to trigger DDoS alert
        "syn_flood_ratio":       3.0,    # SYN:ACK ratio threshold
        "syn_flood_min_syns":    50,     # minimum SYNs before ratio is checked
        "udp_flood_pps":         500,    # UDP packets in the analysis window
        "icmp_flood_count":      100,    # ICMP packets in the analysis window
        "port_scan_ports":       15,     # unique ports in port_scan_window_s seconds
        "port_scan_window_s":    10,     # sliding window for port scan detection
        "brute_force_attempts":  30,     # packets to brute-force ports in 60s
        "dns_entropy_threshold": 3.5,
        "dns_query_rate":        50,
        "beacon_min_intervals":  8,
        "beacon_cv_threshold":   0.15,
        "payload_entropy_high":  7.2,
        # --- COOLDOWN: seconds before the same alert fires again for the same IP ---
        "alert_cooldown_s":      120,    # general cooldown
        "flood_cooldown_s":      60,     # shorter for floods (they're transient)
        "scan_cooldown_s":       120,
        "brute_cooldown_s":      180,
        "ml_cooldown_s":         300,
        # --- MISC ---
        "alert_history_max":     1000,
        "ddos_window_s":         5,      # window for pps calculation
    },
    "ml": {
        "ensemble_contamination": 0.05,
        "ueba_window_hours":      24,
...
def build_logger(name, log_file, level):
    logger = logging.getLogger(name)
    logger.setLevel(getattr(logging, level.upper(), logging.INFO))
    if logger.handlers:
        return logger
...
# ─────────────────────────────────────────────────────────────────────
class ThreatIntelligence:
    def __init__(self, blocklist_path=None, geoip_db=None):
        self._blocklist_ips:  set  = set()
        self._blocklist_nets: list = []
        ...
        with open(path) as f:
            for line in f:
                ...
                try:
                    ...
                    pass
        print(f"🛡  Threat intel: {count} entries")
    ...
    def update_reputation(self, ip, delta):
        with self._lock:
            self._reputation[ip] = max(0.0, min(1.0, self._reputation.get(ip, 0.0) + delta))
...
class UEBAProfile:
    ...
    def observe(self, ts, dst_ip, dst_port, pkt_size, proto):
        cutoff = ts - self.WINDOW
        ...
        self._timestamps.append((ts, ts))
        self._dst_ports.append((ts, dst_port))
        recent = sum(1 for t, _ in self._timestamps if t > ts - 60)
        cur_pps = recent / 60.0
        if self.baseline_pps is None:
            self.baseline_pps = cur_pps
        else:
    ...
    def get_anomaly_score(self, now):
        recent = [t for t, _ in self._timestamps if t > now - 60]
        if len(recent) < 10:
            return 0.0, ""
        cur_pps  = len(recent) / 60.0
        baseline = self.baseline_pps or cur_pps
        if baseline < 0.1:
            return 0.0, ""
        ratio = cur_pps / baseline
        if ratio > 10:
            return min(ratio / 20, 1.0), f"Traffic spike {ratio:.1f}x above baseline"
        ports_now = set(p for t, p in self._dst_ports if t > now - 60)
        if len(ports_now) > 30:
            return 0.85, f"Sudden port diversity: {len(ports_now)} unique ports in 60s"
        return 0.0, ""
...
# ─────────────────────────────────────────────────────────────────────
class DNSAnalyser:
    def __init__(self, entropy_thresh=3.5, query_rate=50):
        self.entropy_thresh = entropy_thresh
        self.query_rate     = query_rate
        self._queries:    Dict[str, deque] = defaultdict(lambda: deque(maxlen=500))
        self._subdomains: Dict[str, set]   = defaultdict(set)
    ...
    def process(self, src_ip, qname, ts):
        qname  = qname.rstrip('.')
        parts  = qname.split('.')
        domain = '.'.join(parts[-2:]) if len(parts) >= 2 else qname
        ...
        with self._lock:
            self._queries[src_ip].append(ts)
            if sub:
                self._subdomains[domain].add(sub)
        findings = {}
        entropy = self._entropy(sub) if sub else 0
        if entropy > self.entropy_thresh and len(sub) > 20:
            findings['dns_tunnel'] = {'reason': 'High-entropy subdomain',
                                      'entropy': round(entropy, 3), 'qname': qname}
        with self._lock:
            recent_rate = sum(1 for t in self._queries[src_ip] if t > ts - 10)
        if recent_rate > self.query_rate:
            findings['dns_flood'] = {'reason': 'DNS query rate exceeded', 'rate': recent_rate}
        n_sub = len(self._subdomains.get(domain, set()))
        if n_sub > 200:
            findings['dns_dga'] = {'reason': 'Excessive unique subdomains (DGA?)',
                                   'unique_subs': n_sub, 'domain': domain}
        return findings if findings else None
...
# ─────────────────────────────────────────────────────────────────────
class BeaconDetector:
    def __init__(self, min_intervals=8, cv_threshold=0.15):
        self.min_intervals = min_intervals
        self.cv_threshold  = cv_threshold
        self._flows: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))
        self._lock = threading.Lock()

    def observe(self, key, ts):
        with self._lock:
            self._flows[key].append(ts)
    ...
    def check(self, key):
        ...
        std_iv  = statistics.stdev(intervals) if len(intervals) > 1 else 0
        cv      = std_iv / mean_iv if mean_iv > 0 else 1.0
        if cv < self.cv_threshold and mean_iv > 1:
            return {'beacon_interval_s': round(mean_iv, 2), 'cv': round(cv, 4),
                    'samples': len(intervals),
                    'description': f'Machine-like {mean_iv:.1f}s beacon (CV={cv:.3f})'}
        return None
    ...
    def observe(self, ip, mac):
        with self._lock:
            ...
            if known is None:
                ...
                return None
            if known.lower() != mac.lower():
                return {'ip': ip, 'known_mac': known, 'claimed_mac': mac,
                        'description': f'ARP spoofing: {ip} moved from {known} to {mac}'}
        return None
...
class EnsembleAnomalyDetector:
    ...
    def __init__(self, contamination=0.05):
        self.contamination = contamination
        self._scaler  = RobustScaler()
        self._iforest = IsolationForest(n_estimators=200, contamination=contamination,
                                        random_state=42, n_jobs=-1)
        self._lof     = LocalOutlierFactor(n_neighbors=20, contamination=contamination,
                                           novelty=True, n_jobs=-1)
        self._ocsvm   = OneClassSVM(kernel='rbf', nu=contamination, gamma='scale')
        self.trained  = False
        self._history: List[np.ndarray] = []
        self._lock    = threading.Lock()
    ...
    def predict(self, X):
        ...
        votes   = np.stack([(p1==-1),(p2==-1),(p3==-1)], axis=1)
        flagged = votes.sum(axis=1) >= 2
        return flagged, (s1+s2+s3)/3.0, votes.astype(int)

    def detect_drift(self, X, threshold=2.0):
        if not self.trained or not self._history:
            return None
        with self._lock:
            train_mean = np.vstack(self._history).mean(axis=0)
        z_max = np.max(np.abs(X.mean(axis=0) - train_mean) / (np.abs(train_mean) + 1e-9))
        if z_max > threshold:
            return f"Feature drift detected (max z={z_max:.2f})"
        return None
...
# ─────────────────────────────────────────────────────────────────────
class AdaptiveThresholds:
    def __init__(self, base, alpha=0.05):
        self._values: Dict[str, float] = {k: float(v) for k, v in base.items()
                                           if isinstance(v, (int, float))}
        self._alpha = alpha
        self._lock  = threading.Lock()

    def update(self, key, observed):
        with self._lock:
            if key in self._values:
                self._values[key] = self._alpha * observed + (1-self._alpha)*self._values[key]
...
class ThreatScoreBoard:
    ...
    def add_evidence(self, ip, weight, label):
        with self._lock:
            self._scores[ip] = min(100.0, self._scores[ip] + weight)
            if label:
                self._evidence[ip].append(label)
    ...
    def top_threats(self, n=10):
        with self._lock:
            return sorted(
                [(ip, sc, self._evidence.get(ip, [])) for ip, sc in self._scores.items()],
                key=lambda x: -x[1]
            )[:n]
...
class WebhookNotifier:
    ...
    def _send(self, cfg, alert):
        url  = cfg.get('url', '')
        kind = cfg.get('type', 'generic').lower()
        payload = ({"text": f"🚨 *{alert['attack_type']}* from `{alert['src_ip']}`"}
                   if kind == 'slack' else alert)
        data = json.dumps(payload).encode()
        req  = urllib.request.Request(url, data=data,
                                      headers={"Content-Type": "application/json"},
                                      method="POST")
...
# ─────────────────────────────────────────────────────────────────────
class PCAPManager:
    def __init__(self, pcap_dir="pcap_captures", max_packets_per_file=10000, max_files=20):
        self.pcap_dir   = pcap_dir
        self.max_pkt    = max_packets_per_file
        self.max_files  = max_files
        sess = f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.session_dir = os.path.join(pcap_dir, sess)
        os.makedirs(self.session_dir, exist_ok=True)
        self._lock = threading.Lock()
        self._idx  = 1
        self._buf: List = []
...
class PCAPFeatureExtractor:
    FEATURE_NAMES = [
        "pkt_count","byte_total","avg_pkt_size","std_pkt_size",
        "duration","pps","bps","tcp_ratio","udp_ratio","icmp_ratio",
        "syn_ratio","rst_ratio","fin_ratio","unique_dst_ports","unique_dst_ips",
        "avg_payload_entropy","beacon_cv","dns_ratio","malicious_port_hits","small_pkt_ratio",
    ]
    ...
    def extract(self, packets):
        ...
        for pkt in packets:
            ...
            src = pkt[IP].src
            if src not in flows:
                flows[src] = {"sizes":[],"times":[],"payloads":[],
                              "tcp":0,"udp":0,"icmp":0,"dns":0,
                              "syn":0,"rst":0,"fin":0,
                              "dst_ports":set(),"dst_ips":set(),
                              "mal_ports":0,"small_pkts":0}
            ...
            if TCP in pkt:
                ...
                port = pkt[TCP].dport
                f["dst_ports"].add(port)
                if port in KNOWN_MALICIOUS_PORTS: f["mal_ports"] += 1
            elif UDP in pkt:
                f["udp"] += 1
                port = pkt[UDP].dport
                f["dst_ports"].add(port)
                if DNS in pkt: f["dns"] += 1
            elif ICMP in pkt:
                f["icmp"] += 1
        ...
        for src_ip, f in flows.items():
            ...
            rows.append([
                ...
                len(f["dst_ports"]), len(f["dst_ips"]),
                np.mean(f["payloads"]) if f["payloads"] else 0.0,
                cv, f["dns"]/n, float(f["mal_ports"]), f["small_pkts"]/n,
            ])
            ips.append(src_ip)
        return np.array(rows, dtype=np.float32), ips
...
# ─────────────────────────────────────────────────────────────────────
class MetricsAPI:
    def __init__(self, ids_ref, port=9001):
        self._ids    = ids_ref
        self._port   = port
        self._server = None
...
class AdvancedNetworkIDS:
    ...
    def __init__(self, config: dict):
        ...
        self.pcap_mgr     = PCAPManager(config["pcap_dir"], max_files=config["max_pcap_files"])
        self.extractor    = PCAPFeatureExtractor()
        self.ensemble     = EnsembleAnomalyDetector(self.ml_cfg["ensemble_contamination"])
        self.ueba:         Dict[str, UEBAProfile] = {}
        self.dns_analyser = DNSAnalyser(self.thr["dns_entropy_threshold"], self.thr["dns_query_rate"])
        self.beacon_det   = BeaconDetector(self.thr["beacon_min_intervals"], self.thr["beacon_cv_threshold"])
        self.arp_det      = ARPSpoofDetector()
        self.threat_intel = ThreatIntelligence(config.get("threat_intel_file"), config.get("geoip_db"))
        self.scoreboard   = ThreatScoreBoard()
        self.adapt_thr    = AdaptiveThresholds(self.thr)
        self.notifier     = WebhookNotifier(config.get("webhooks", []))

        self.logger = build_logger("AdvancedIDS", config.get("log_file","network_ids.log"),
                                   config.get("log_level","INFO"))
        ...
        # ── COOLDOWN TRACKING ──────────────────────────────────────────
        # Key: "{alert_type}__{src_ip}" → unix timestamp of last alert
        # Must be initialized to empty dict (NOT defaultdict(float)),
        # so we can tell the difference between "never alerted" (missing key)
        # and "alerted at time 0" (which would defeat cooldowns on startup).
        self._cooldown: Dict[str, float] = {}
        ...
        self._whitelist_nets: list = []
        for cidr in config.get("whitelist_cidrs", []):
            try: self._whitelist_nets.append(ipaddress.ip_network(cidr, strict=False))
    ...
    def _check_arp(self, pkt):
        ...
        if arp.op == 2:
            finding = self.arp_det.observe(arp.psrc, arp.hwsrc)
            if finding:
                self._alert('ARP Spoofing', arp.psrc, 'Critical', 0.97,
                            finding, cooldown_key='arp', score_weight=30)
    ...
    def _process_ip(self, pkt):
        ...
        if self.threat_intel.is_blocked(src):
            ...
            self._alert('Threat-Intel Blocklist Hit', src, 'Critical',
                        min(0.99, 0.80 + rep*0.19),
                        {'reputation_score': round(rep, 3),
                         'description': 'IP matched threat intelligence blocklist'},
                        cooldown_key='blocklist', score_weight=40)
        ...
        if TCP in pkt:
            ...
            if Raw in pkt:
                ...
                if label:
                    self._alert('Suspicious Payload Entropy', src, 'Medium', 0.80,
                                {'entropy': round(ent, 3), 'dport': dport, 'reason': label},
                                cooldown_key='payload_entropy', score_weight=10)
            ...
            if dport in KNOWN_MALICIOUS_PORTS:
                self._alert('Malicious Port Contact', src, 'High', 0.85,
                            {'dport': dport,
                             'description': f'Connection to known C2/backdoor port {dport}'},
                            cooldown_key=f'malport_{dport}', score_weight=20)
            ...
            if DNS in pkt and DNSQR in pkt:
                ...
                if findings:
                    for ftype, fdata in findings.items():
                        sev = 'High' if ftype == 'dns_tunnel' else 'Medium'
                        self._alert(f'DNS Anomaly ({ftype})', src, sev, 0.82,
                                    fdata, cooldown_key=f'dns_{ftype}', score_weight=15)
        ...
        if src not in self.ueba:
            self.ueba[src] = UEBAProfile(src)
        self.ueba[src].observe(now, pkt[IP].dst, dport, size, proto)

    def _inspect_http(self, src_ip, pkt):
        raw   = bytes(pkt[Raw].load).decode(errors='replace')
        lines = raw.split('\r\n')
        if not lines: return
        req_line = lines[0]
        ...
        if any(p in raw.lower() for p in sqli):
            self._alert('HTTP SQLi Attempt', src_ip, 'High', 0.80,
                        {'request': req_line[:200],
                         'description': 'SQL injection pattern in HTTP request'},
                        cooldown_key='sqli', score_weight=25)

        if '../' in raw or '%2e%2e' in raw.lower():
            self._alert('HTTP Path Traversal', src_ip, 'High', 0.82,
                        {'request': req_line[:200],
                         'description': 'Directory traversal attempt'},
                        cooldown_key='path_traversal', score_weight=20)

        scanners = ['sqlmap','nikto','nmap','masscan','zgrab','dirbuster','gobuster','burpsuite']
        ua = next((l for l in lines if l.lower().startswith('user-agent:')), '')
        if any(s in ua.lower() for s in scanners):
            self._alert('HTTP Scanner Detected', src_ip, 'High', 0.90,
                        {'user_agent': ua[:200], 'description': 'Known security scanner UA'},
                        cooldown_key='scanner_ua', score_weight=20)

    # ═════════════════════════════════════════════════════════════════
    # ANALYSIS LOOP
    # Runs every 2 seconds and checks all signature-based detections.
    # Each detector RESETS its counters after firing, so it can only
    # alert again once the counters accumulate fresh evidence.
    # ═════════════════════════════════════════════════════════════════
    def _analysis_loop(self):
        while self.running:
            try:
    ...
    def _detect_ddos(self):
        now    = time.time()
        win    = self.thr.get('ddos_window_s', 5)
        thresh = self.adapt_thr.get('ddos_pps', self.thr['ddos_pps'])

        for ip, c in list(self.connections.items()):
            if self._whitelisted(ip): continue
            recent = [t for t in c['ts'] if t > now - win]
            pps    = len(recent) / win
            # Update adaptive threshold with observed (normal) pps
            if pps > 0:
                self.adapt_thr.update('ddos_pps', pps * 3)  # 3x observed = threshold
            if pps > thresh:
                self._alert('DDoS / High-Rate Flood', ip, 'Critical', 0.95,
                            {'pps': f"{pps:.0f}", 'threshold': f"{thresh:.0f}",
                             'description': 'Packet rate far exceeds baseline threshold'},
                            cooldown_key='ddos',
                            cooldown_override=self.thr.get('flood_cooldown_s', 60),
                            score_weight=35)
                # NOTE: We do NOT clear c['ts'] here — those timestamps are still real.
                # The cooldown prevents re-alerting; adaptive threshold will raise.

    # ── SYN Flood ────────────────────────────────────────────────────
    def _detect_syn_flood(self):
        thresh_ratio = self.adapt_thr.get('syn_flood_ratio', self.thr['syn_flood_ratio'])
        min_syns     = self.thr.get('syn_flood_min_syns', 50)
        ...
        for ip, c in list(self.connections.items()):
            ...
            # Only evaluate when we have enough SYNs to be meaningful
            if syn < min_syns:
                continue
            ...
            if ratio > thresh_ratio:
                self._alert('SYN Flood', ip, 'Critical', 0.96,
                            {'syn': syn, 'ack': ack,
                             'ratio': f"{ratio:.2f}",
                             'description': 'Half-open SYN flood (high SYN:ACK ratio)'},
                            cooldown_key='syn_flood',
                            cooldown_override=self.thr.get('flood_cooldown_s', 60),
                            score_weight=30)
                # Reset counters so alert re-arms only after fresh evidence
                c['syn'] = 0
                c['ack'] = 0

    # ── UDP Flood ────────────────────────────────────────────────────
    def _detect_udp_flood(self):
        # udp_flood_pps is treated as a count-per-analysis-window threshold
        thresh = self.adapt_thr.get('udp_flood_pps', self.thr['udp_flood_pps'])
        ...
        for ip, c in list(self.connections.items()):
            ...
            if c['udp'] > thresh:
                self._alert('UDP Flood', ip, 'High', 0.92,
                            {'udp_pkts': c['udp'],
                             'description': f"UDP packet count {c['udp']} exceeds threshold {thresh:.0f}"},
                            cooldown_key='udp_flood',
                            cooldown_override=self.thr.get('flood_cooldown_s', 60),
                            score_weight=25)
                # Reset counter — alert re-arms after counter fills again
                c['udp'] = 0

    # ── ICMP Flood ───────────────────────────────────────────────────
    def _detect_icmp_flood(self):
        thresh = self.adapt_thr.get('icmp_flood_count', self.thr.get('icmp_flood_count', 100))
        ...
        for ip, c in list(self.connections.items()):
            ...
            if c['icmp'] > thresh:
                self._alert('ICMP Flood', ip, 'Medium', 0.90,
                            {'icmp_pkts': c['icmp'],
                             'description': f"ICMP count {c['icmp']} exceeds threshold {thresh:.0f}"},
                            cooldown_key='icmp_flood',
                            cooldown_override=self.thr.get('flood_cooldown_s', 60),
                            score_weight=15)
                # Reset counter
                c['icmp'] = 0
    ...
    def _detect_port_scans(self):
        ...
        Logic:
          - Track unique ports contacted within the last port_scan_window_s seconds
          - Use a dedicated per-IP deque of (timestamp, port) instead of a
            simple set, so the window slides correctly
          - Alert when unique-port-count in window >= threshold
          - After alerting, clear the port-timestamp deque to re-arm
        """
        now    = time.time()
        win    = self.thr.get('port_scan_window_s', 10)
        thresh = self.thr['port_scan_ports']

        for ip, c in list(self.connections.items()):
            if self._whitelisted(ip): continue

            # Prune old port-timestamp entries outside the window
            while c['port_ts'] and c['port_ts'][0][0] < now - win:
                c['port_ts'].popleft()

            unique_ports = set(p for _, p in c['port_ts'])
            if len(unique_ports) >= thresh:
                self._alert('Port Scan', ip, 'High', 0.93,
                            {'ports_scanned': len(unique_ports),
                             'sample_ports': sorted(unique_ports)[:20],
                             'window_s': win,
                             'description': f'{len(unique_ports)} unique ports in {win}s window'},
                            cooldown_key='port_scan',
                            cooldown_override=self.thr.get('scan_cooldown_s', 120),
                            score_weight=25)
                # Clear so it re-arms from zero after cooldown
                c['port_ts'].clear()

    # ── Brute Force ──────────────────────────────────────────────────
    def _detect_brute_force(self):
        """
        Detects brute force against common authentication services.

        Logic:
          - Count packets to brute-force-relevant ports in a 60s sliding window
          - Only fire when the count is genuinely high AND the IP is repeatedly
            hitting the same auth port
          - Reset the brute-force counter after alerting
        """
        brute_ports = {22, 21, 23, 3389, 3306, 5432, 1433, 5900}
        now    = time.time()
        window = 60
        thresh = self.thr['brute_force_attempts']
        ...
        for ip, c in list(self.connections.items()):
            ...
            # Which brute-force ports has this IP hit?
            active_brute_ports = c['ports'] & brute_ports
            if not active_brute_ports:
                continue

            # Count recent packets toward those specific ports
            recent_count = c.get('brute_recent', 0)
            # Use a per-IP sliding window counter, stored separately
            recent_pkts = [t for t in c['ts'] if t > now - window]
            if len(recent_pkts) < thresh:
                continue

            # Extra sanity: packet rate to these ports must be significant
            # (avoid alerting on servers that legitimately get many SSH connections)
            rate_pps = len(recent_pkts) / window
            if rate_pps < 0.5:      # less than 1 packet every 2s → not brute force
                continue

            self._alert('Brute Force', ip, 'High', 0.88,
                        {'attempts_in_60s': len(recent_pkts),
                         'rate_pps': f"{rate_pps:.1f}",
                         'ports': sorted(active_brute_ports),
                         'services': self._services(active_brute_ports),
                         'description': f'High-rate authentication attempts on {self._services(active_brute_ports)}'},
                        cooldown_key='brute_force',
                        cooldown_override=self.thr.get('brute_cooldown_s', 180),
                        score_weight=30)
            # Don't clear c['ts'] — keep timestamps for other detectors.
            # The cooldown ensures we don't re-alert until 3 minutes pass.
    ...
    def _run_live_ml(self):
        ...
        for ip, c in list(self.connections.items()):
            ...
            pps   = c['pkt_count'] / dur
            bps   = c['bytes'] / dur
            avg   = float(np.mean(list(c['sizes']))) if c['sizes'] else 0
            std   = float(np.std(list(c['sizes'])))  if len(c['sizes']) > 1 else 0
            syn_r = c['syn'] / max(c['pkt_count'], 1)
            ports = len(c['ports'])
            tu    = c['tcp'] / max(c['udp'], 1)
            dns_r = c['dns'] / max(c['pkt_count'], 1)
        ...
        if self.ensemble.trained and X:
            ...
            for i, ip in enumerate(ips):
                if flagged[i]:
                    self._alert('ML Ensemble Anomaly', ip, 'Medium',
                                min(0.99, abs(float(scores[i])) + 0.3),
                                {'if_vote': int(votes[i][0]),
                                 'lof_vote': int(votes[i][1]),
                                 'ocsvm_vote': int(votes[i][2]),
                                 'ensemble_score': round(float(scores[i]), 4),
                                 'description': '2/3 models flagged abnormal traffic profile'},
                                cooldown_key='ml_live',
                                cooldown_override=self.thr.get('ml_cooldown_s', 300),
                                score_weight=20)
                    self.threat_intel.update_reputation(ip, 0.1)
    ...
    def _ueba_loop(self):
        while self.running:
            ...
            for ip, profile in list(self.ueba.items()):
                ...
                score, reason = profile.get_anomaly_score(now)
                if score > 0.7:
                    self._alert('UEBA Behavioural Anomaly', ip, 'Medium', score,
                                {'reason': reason,
                                 'baseline_pps': round(profile.baseline_pps or 0, 2),
                                 'description': 'Behaviour drifted significantly from 24h baseline'},
                                cooldown_key='ueba',
    ...
    def _run_pcap_ml(self):
        ...
        if not pkts: return
        X, ips = self.extractor.extract(pkts)
        if len(X) < self.ensemble.MIN_SAMPLES: return
        ...
        for i, ip in enumerate(ips):
            if flagged[i] and not self._whitelisted(ip):
                self._alert('PCAP ML Anomaly', ip, 'High',
                            min(0.99, abs(float(scores[i])) + 0.4),
                            {'ensemble_score': round(float(scores[i]), 4),
                             'description': 'PCAP-level ensemble flagged abnormal flow'},
                            cooldown_key='ml_pcap',
                            cooldown_override=self.thr.get('ml_cooldown_s', 300),
                            score_weight=20)
                ...
                if score >= 70:
                    self._alert('Composite High-Risk IP', ip, 'Critical', 0.95,
                                {'composite_score': round(score, 1),
                                 'evidence_count': len(evidence),
                                 'top_evidence': list(set(evidence))[:5],
                                 'reputation': round(self.threat_intel.get_reputation(ip), 3),
                                 'description': 'Accumulated threat score across multiple detectors'},
                                cooldown_key='composite_score',
                                cooldown_override=300,
                                score_weight=0)  # don't double-count
    ...
    def _alert(self,
               ...
               *,
               ...
               score_weight: float = 10):
        """
        Fire an alert for (attack_type, src_ip) subject to cooldown.

        Cooldown key is "{cooldown_key}__{src_ip}" so each IP has its
        own independent cooldown per attack type.  cooldown_override lets
        individual detectors specify a different cooldown period.
        """
        now = time.time()
        cd  = cooldown_override if cooldown_override is not None \
              else self.thr.get('alert_cooldown_s', 120)

        # Build a stable key: use cooldown_key (short label) + IP
        ckey = f"{cooldown_key or attack_type}__{src_ip}"
        last = self._cooldown.get(ckey)   # returns None if never alerted
        if last is not None and (now - last) < cd:
            return  # within cooldown window → suppress

        self._cooldown[ckey] = now  # record this alert timestamp
        ...
        self._save_alert(alert)
        self.notifier.notify(alert)
        self.logger.warning(f"{attack_type} | {src_ip}", extra={'src_ip': src_ip, 'attack_type': attack_type})

    # ═════════════════════════════════════════════════════════════════
    # HELPERS
    # ═════════════════════════════════════════════════════════════════
    def _whitelisted(self, ip):
        if ip in self._whitelist_ips: return True
        if any(ip.startswith(p) for p in PRIVATE_PREFIXES): return True
        try:
            addr = ipaddress.ip_address(ip)
            return any(addr in net for net in self._whitelist_nets)
        except ValueError:
    ...
    def _new_conn():
        ...
        return {
            ...
            'ts':      deque(maxlen=500),
            # Sliding-window port scan tracker: (timestamp, port) pairs
            'port_ts': deque(maxlen=1000),
            'first_seen': now,
            'last_seen':  now,
        }

    # ── Patch: record (ts, port) into port_ts on every TCP/UDP packet ──
    # This is called from _process_ip; we override the connection update
    # there to also append to port_ts.

    def _record_port(self, src_ip: str, ts: float, port: int):
        """Call this whenever a packet destined for a new port is seen."""
        self.connections[src_ip]['port_ts'].append((ts, port))

    def _cleanup_stale(self):
        cutoff = time.time() - 300
        for ip in [ip for ip, c in list(self.connections.items()) if c['last_seen'] < cutoff]:
            del self.connections[ip]
            self.ueba.pop(ip, None)

    def _serialise(self, obj):
        if isinstance(obj, dict):   return {str(k): self._serialise(v) for k, v in obj.items()}
        if isinstance(obj, (list, tuple)): return [self._serialise(v) for v in obj]
    ...
    def _services(self, ports):
        return ', '.join(SERVICE_MAP.get(p, f'port/{p}') for p in sorted(ports))
    ...
    def _detect_interface():
        try:
            for iface in get_if_list():
                ip = get_if_addr(iface)
                if not ip or ip.startswith(('127.','0.0.0.0','169.254.')): continue
                if ip.startswith(('192.168.','10.','172.')):
                    print(f"✓ Auto-selected interface: {iface} ({ip})")
                    return iface
        except Exception:
    ...
    def _prometheus_metrics(self):
        ...
        lines = [
            ...
            '# TYPE ids_alerts_total counter',
            f'ids_alerts_total {s["alerts"]}',
            '# HELP ids_unique_ips Unique source IPs',
            '# TYPE ids_unique_ips gauge',
            f'ids_unique_ips {len(self.connections)}',
            f'ids_ml_trained {int(self.ensemble.trained)}',
        ]
        for atype, cnt in self.alert_counts.items():
            safe = re.sub(r'[^a-zA-Z0-9_]', '_', atype).lower()
            lines += [f'# TYPE ids_alert_{safe}_total counter',
                      f'ids_alert_{safe}_total {cnt}']
        return '\n'.join(lines) + '\n'
    ...
    def _json_stats(self):
        up = (datetime.now() - self.stats['start_time']).total_seconds()
        return {
            'version': VERSION, 'uptime_s': round(up, 1), 'interface': self.interface,
            **{k: (v.isoformat() if isinstance(v, datetime) else v) for k, v in self.stats.items()},
            'unique_ips': len(self.connections),
            'ml_trained': self.ensemble.trained,
            'pcap_files': len(self.pcap_mgr.list_files()),
            'alert_counts': dict(self.alert_counts),
    ...
    def _json_threats(self):
        return [{'ip': ip, 'score': score, 'evidence': list(set(ev))[:10]}
                for ip, score, ev in self.scoreboard.top_threats(20)]
    ...
    def _print_banner(self):
        print(f"""
╔══════════════════════════════════════════════════════════════╗
║   Advanced Network IDS  v{VERSION}  —  Fixed Edition         ║
╠══════════════════════════════════════════════════════════════╣
║  Interface  : {self.interface:47s}║
║  Alert File : {self.cfg['alert_file']:47s}║
║  Cooldowns  : general={self.thr.get('alert_cooldown_s',120)}s  flood={self.thr.get('flood_cooldown_s',60)}s  brute={self.thr.get('brute_cooldown_s',180)}s   ║
╠══════════════════════════════════════════════════════════════╣
║  Engines: SigDet  ML-Ensemble  UEBA  Beaconing  DNS  ARP    ║
╚══════════════════════════════════════════════════════════════╝
""")
    ...
    def _print_status(self):
        ...
        pps = s['total_packets'] / max(up, 1)
        top = self.scoreboard.top_threats(1)
        ts  = f"TopThreat:{top[0][0]}({top[0][1]:.0f})" if top else ""
        print(f"\r📊 Pkts:{s['total_packets']:,} {pps:.0f}pps "
              f"IPs:{len(self.connections)} Alerts:{s['alerts']} "
              f"ML:{'✓' if self.ensemble.trained else '…'} {ts}",
              end='', flush=True)

    def _print_stats(self):
        up = (datetime.now() - self.stats['start_time']).total_seconds()
        print(f"\n{'═'*62}")
        print(f"  Session Summary  ({up/60:.1f} min)")
        print(f"{'═'*62}")
        ...
        for at, cnt in sorted(self.alert_counts.items(), key=lambda x: -x[1]):
            print(f"    {at:50s} {cnt:5d}")
        print(f"\n  Top Threats:")
        for ip, sc, ev in self.scoreboard.top_threats(5):
            print(f"    {ip:20s} score={sc:.0f}  {list(set(ev))[:3]}")
        print('═'*62)


# ─────────────────────────────────────────────────────────────────────
# ALSO: patch _process_ip to record ports into port_ts
# ─────────────────────────────────────────────────────────────────────
_orig_process_ip = AdvancedNetworkIDS._process_ip

def _patched_process_ip(self, pkt):
    _orig_process_ip(self, pkt)
    # After normal processing, record port for scan detection
    if IP in pkt:
        src  = pkt[IP].src
        now  = time.time()
        port = None
        ...
        if port is not None:
            self.connections[src]['port_ts'].append((now, port))

AdvancedNetworkIDS._process_ip = _patched_process_ip
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description=f'Advanced Network IDS v{VERSION}',
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ap.add_argument('-c','--config',     help='YAML config file')
    ap.add_argument('-i','--interface',  help='Network interface')
    ap.add_argument('-o','--output',     default='network_alerts.json')
    ...
    ap.add_argument('--threat-intel',    help='IP blocklist file')
    ap.add_argument('--geoip-db',        help='MaxMind GeoIP2 .mmdb path')
    ap.add_argument('--analyse-pcap',    help='Offline PCAP analysis mode')
    ap.add_argument('--no-api',          action='store_true')
    args = ap.parse_args()

    if args.analyse_pcap:
        print(f"\n🔍 Offline PCAP: {args.analyse_pcap}")
        pkts = rdpcap(args.analyse_pcap)
